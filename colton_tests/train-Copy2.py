...
class TrainingConfig:
    ...
    instance_prompt: str = "a [V] man"
    class_prompt: str = "a man"
    evaluate_prompt = ["a man"] * 4 + ["a [V] man"] * 1 + ["margot robbie"] * 1 + ["concept art of a [V] man by carne griffiths"] * 2
    ...
    sample_guidance_scale: float = 10 # guidance scale at inference

    # Practical Training Settings
    mixed_precision: str = 'fp16'  # `no` for float32, `fp16` for automatic mixed precision
    save_image_epochs: int = 1
    save_model_epochs: int = 1
    output_dir: str = 'logs/coltontest5'
